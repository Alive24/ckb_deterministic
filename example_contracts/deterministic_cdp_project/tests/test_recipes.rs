...
mod tests {
    ...
    use ckb_deterministic::{
        cell_classifier::{
            CellClass, CellClassifier, CellInfo, ClassificationRule, ClassifiedCells,
            RuleBasedClassifier,
        },
        errors::Error,
    ...
    use deterministic_cdp_shared::cdp_types::{CDP_ADJUST_VAULT, CDP_CLOSE_VAULT, CDP_OPEN_VAULT};
    ...
    fn test_open_vault_validation() {
        ...
        let context =
            create_mock_transaction_context(CDP_OPEN_VAULT, input_cells, output_cells).unwrap();

        // Validation should pass
        let rules = TransactionValidationRules::new(CDP_OPEN_VAULT.to_vec()).with_custom_cell(
            "vault",
    ...
    fn test_open_vault_wrong_method_path() {
        ...
        let context =
            create_mock_transaction_context(b"wrongMethod", input_cells, output_cells).unwrap();
    ...
    fn test_close_vault_validation() {
        ...
        let context =
            create_mock_transaction_context(CDP_CLOSE_VAULT, input_cells, output_cells).unwrap();

        let rules = TransactionValidationRules::new(CDP_CLOSE_VAULT.to_vec()).with_custom_cell(
            "vault",
    ...
    fn test_adjust_vault_validation() {
        ...
        let context =
            create_mock_transaction_context(CDP_ADJUST_VAULT, input_cells, output_cells).unwrap();

        let rules = TransactionValidationRules::new(CDP_ADJUST_VAULT.to_vec()).with_custom_cell(
            "vault",
            CellCountConstraint::exactly(1), // One vault in inputs
            CellCountConstraint::exactly(1), // One vault in outputs
        );
    ...
    fn test_business_rule_validation() {
                        ...
                        Some(vault_cells) => {
                            for vault in vault_cells {
                                ...
                                if vault.data.len() >= 48 {
                                    let collateral =
                                        u128::from_le_bytes(vault.data[32..48].try_into().unwrap());
                                    if collateral < 100 * 10u128.pow(8) {
                                        // Minimum 100 units
        ...
        let context =
            create_mock_transaction_context(b"customTest", input_cells, output_cells).unwrap();

        assert_eq!(
            custom_rules.validate(&context),
