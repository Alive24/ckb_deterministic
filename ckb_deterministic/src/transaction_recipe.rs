...
use crate::generated::{
    Byte32, Byte32Vec, Byte32VecOpt, Bytes, CellDep, CellDepVec, CellDepVecOpt, OutPoint,
    RecipeArgument, RecipeArgumentVec, TransactionRecipe, Uint32,
};
...
impl TransactionRecipeExt for TransactionRecipe {
    ...
    fn method_path_name(&self) -> Result<String, Error> {
        ...
        String::from_utf8(method_path_bytes.to_vec()).map_err(|_| Error::DataError)
    }
    ...
    fn get_argument_type(&self, index: usize) -> Option<u8> {
        self.get_argument(index)
            .map(|arg| arg.arg_type().as_slice()[0])
    }
...
pub fn create_transaction_recipe(
    method_name: &str,
    arguments: &[Vec<u8>],
    ...
    loop {
        match high_level::load_witness(index, Source::Input) {
            Ok(data) => {
                ...
                debug_trace!("Loading witness at index: {}", index);
                debug_trace!("Witness data: {:?}", data);
...
pub fn parse_transaction_recipe_at(index: usize) -> Result<Option<TransactionRecipe>, Error> {
    let witness_data =
        high_level::load_witness(index, Source::Input).map_err(|_| Error::DataError)?;
...
pub fn create_recipe_with_args(
    ...
    let arguments = RecipeArgumentVec::new_builder().set(args).build();
...
pub fn create_transaction_recipe_with_deps(
    params: RecipeParams,
) -> Result<TransactionRecipe, Error> {
    // Convert method path name to bytes
    let method_path_bytes = Bytes::new_builder()
...
mod tests {
    ...
    fn test_create_transaction_recipe() {
        ...
        assert_eq!(
            recipe.method_path_hash().unwrap(),
            method_path("UDT.transfer")
        );
    ...
    fn test_serialization() {
        ...
        let deserialized = parse_transaction_recipe_from_data(&witness_data)
            .unwrap()
            .unwrap();
    ...
    fn test_parse_witness_with_recipe() {
        ...
        match result {
            Ok(Some(recipe)) => {
                ...
                // Check method path
                assert_eq!(
                    recipe.method_path_name().unwrap(),
    ...
    fn test_create_recipe_with_output_reference() {
        ...
        let witness_data = witness_args.as_bytes();
        let parsed = parse_transaction_recipe_from_data(&witness_data)
            .unwrap()
            .unwrap();

        assert_eq!(
            parsed.method_path_name().unwrap(),
            "CKBoostProtocol.update_protocol"
        );
    }
    ...
    fn test_analyze_recipe_error() {
        ...
        let recipe = parse_transaction_recipe_from_data(&witness_bytes)
            .unwrap()
            .unwrap();
        ...
        println!(
            "1. Output cell at index {} might not exist in the transaction",
            index
        );
        println!(
            "2. The output cell at index {} might not have any data",
            index
        );
        println!(
            "3. The data at output index {} might be invalid for the expected format",
            index
        );
