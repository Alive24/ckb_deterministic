...
extern crate alloc;
use alloc::{vec::Vec, collections::BTreeMap, string::ToString};
...
fn test_context_with_classified_cells() {
    ...
    assert_eq!(context.input_cells.get_known("xudt").map(|v| v.len()), Some(1));
    assert_eq!(context.input_cells.get_custom("vault").map(|v| v.len()), Some(1));
...
fn test_context_cell_access_patterns() {
    ...
    input_known.insert("xudt".to_string(), vec![
        create_test_cell(0, Some([10u8; 32])),
        create_test_cell(1, Some([10u8; 32])),
    ]);
    
    let mut input_custom = BTreeMap::new();
    input_custom.insert("vault".to_string(), vec![
        create_test_cell(2, Some([20u8; 32])),
    ]);
    
    let mut output_known = BTreeMap::new();
    output_known.insert("xudt".to_string(), vec![
        create_test_cell(0, Some([10u8; 32])),
    ]);
...
fn test_context_large_cell_collections() {
    ...
    // Create many cells
    let xudt_cells: Vec<_> = (0..100).map(|i| create_test_cell(i, Some([10u8; 32]))).collect();
    let vault_cells: Vec<_> = (100..200).map(|i| create_test_cell(i, Some([20u8; 32]))).collect();
...
fn test_context_mixed_sources() {
    ...
    assert_eq!(context.input_cells.get_simple_ckb()[0].source, Source::Input);
    assert_eq!(context.output_cells.get_simple_ckb()[0].source, Source::Output);
}