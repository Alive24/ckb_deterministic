...
fn create_test_context(
    ...
    let output_cells = ClassifiedCells {
        ...
        known_cells: output_map.iter()
            .filter(|(k, _)| k.as_str() != "simple_ckb" && !k.starts_with("custom_"))
            .map(|(k, v)| (k.clone(), v.clone()))
            .collect(),
        custom_cells: output_map.iter()
            .filter(|(k, _)| k.starts_with("custom_"))
            .map(|(k, v)| (k.strip_prefix("custom_").unwrap().to_string(), v.clone()))
            .collect(),
        unidentified_cells: vec![],
...
fn test_argument_count_validation() {
    ...
    assert_eq!(rules.validate(&context_wrong_args), Err(Error::InvalidArgumentCount));
}
...
fn test_cell_count_constraints() {
    ...
    assert_eq!(rules.validate(&context_fail), Err(Error::CellCountViolation));
}
...
fn test_business_rule_validation() {
    ...
    assert_eq!(rules.validate(&context_fail), Err(Error::BusinessRuleViolation));
}

#[test]
fn test_multiple_validation_rules() {
    let rules: TransactionValidationRules<RuleBasedClassifier> = TransactionValidationRules::new(b"complex".to_vec())
        .with_arguments(2)
        .with_cell_relationship(
...
fn test_range_constraints() {
    ...
    assert_eq!(rules.validate(&context_many), Err(Error::CellCountViolation));
}