...
fn test_range_checks() {
    ...
    assert_eq!(
        expect(0).to_be_in_range(1, 10),
        Err(Error::ExpectationViolation)
    );
    assert_eq!(
        expect(11).to_be_in_range(1, 10),
        Err(Error::ExpectationViolation)
    );
}
...
fn test_option_assertions() {
    ...
    assert_eq!(
        expect(none_value).to_be_some_and_equal(42),
        Err(Error::ExpectationViolation)
    );
}
...
fn test_collection_length() {
    ...
    assert_eq!(
        expect(&empty_vec).to_have_length(1),
        Err(Error::CellCountViolation)
    );
}
...
fn test_custom_predicate() {
    ...
    assert_eq!(
        expect(3).to_satisfy(is_even, "should be even"),
        Err(Error::ExpectationViolation)
    );
}
...
fn test_chained_assertions() {
    ...
    for &num in &vec {
        ...
        assert!(expect(num)
            .to_satisfy(|n| n % 2 == 0, "should be even")
            .is_ok());
    }
}
