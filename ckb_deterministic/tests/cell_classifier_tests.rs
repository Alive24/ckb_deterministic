use ckb_deterministic::cell_classifier::{
    CellClass, CellClassifier, CellInfo, ClassificationRule, RuleBasedClassifier,
};
...
fn test_custom_predicate_classification() {
    let classifier = RuleBasedClassifier::new("TestClassifier").add_custom(
        "specific_token",
        |cell| {
...
fn test_multiple_known_scripts() {
    ...
    // Spore cell
    let spore_cell = create_test_cell(lock_script, Some(create_test_script(spore_hash, vec![])));
