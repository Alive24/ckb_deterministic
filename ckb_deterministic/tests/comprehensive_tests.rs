...
mod comprehensive_tests {
    ...
    use ckb_deterministic::{
        assertions::expect,
        cell_classifier::{
            CellClass, CellClassifier, CellCollector, CellInfo, ClassificationRule,
            ClassifiedCells, RuleBasedClassifier,
        },
        errors::Error,
        generated::{Bytes, RecipeArgumentVec, TransactionRecipeBuilder},
    ...
    fn test_cell_classification_known_script() {
        ...
        let classifier =
            RuleBasedClassifier::new("TestClassifier").add_rule(ClassificationRule::TypeCodeHash {
    ...
    fn test_cell_classification_by_lock_script() {
        ...
        let classifier =
            RuleBasedClassifier::new("TestClassifier").add_rule(ClassificationRule::LockCodeHash {
    ...
    fn test_classified_cells_with_unidentified() {
        let unidentified = vec![create_test_cell(
            Source::Input,
            0,
            [1u8; 32],
            Some([99u8; 32]),
    ...
    fn test_validation_arguments() {
        let rules = TransactionValidationRules::new(b"transfer".to_vec()).with_arguments(2);
    ...
    fn test_validation_cell_count_constraints() {
        ...
        for (constraint, count, should_pass) in test_cases {
            ...
            let cells: Vec<_> = (0..count)
                .map(|i| create_test_cell(Source::Input, i, [1u8; 32], Some([20u8; 32]), vec![]))
                .collect();
            ...
            let context = create_mock_context(b"test", input_cells, ClassifiedCells::default());
    ...
    fn test_assertions_collections() {
        ...
        assert_eq!(
            expect(&empty_vec).not_to_be_empty(),
            Err(Error::CellCountViolation)
        );
    }
    ...
    fn test_assertions_option() {
        ...
        assert_eq!(
            expect(some_value).to_be_some_and_equal(43),
            Err(Error::ExpectationViolation)
        );
    }
    ...
    fn test_edge_case_large_cell_count() {
        let mut custom = BTreeMap::new();
        let large_cell_vec: Vec<_> = (0..1000)
            .map(|i| create_test_cell(Source::Input, i, [1u8; 32], Some([20u8; 32]), vec![]))
            .collect();
        ...
        let context = create_mock_context(b"bulk", input_cells, ClassifiedCells::default());

        let rules = TransactionValidationRules::new(b"bulk".to_vec()).with_custom_cell(
            "bulk",
    ...
    fn test_edge_case_malformed_data() {
        ...
        let rules = TransactionValidationRules::new(b"test".to_vec()).with_business_rule(
    ...
    fn test_error_propagation() {
        ...
        let rules: TransactionValidationRules<RuleBasedClassifier> =
            TransactionValidationRules::new(b"test".to_vec())
                .with_arguments(1)
                .with_cell_relationship(
